...
const adj1500 = 1500 - temp;   // + = add heat, - = subtract heat
const adj1520 = 1520 - temp;

const cls = (v) => v > 0 ? 'pos' : (v < 0 ? 'neg' : 'neu');
const dir = (v) => v > 0 ? 'Add' : (v < 0 ? 'Subtract' : 'No change');
const mag = (v) => Math.abs(v);
...
out.innerHTML = `
  ...
  <table class="table-out" role="table" aria-label="Calibration Adjustments">
    <thead>
      <tr><th>Baseline</th><th>Action</th><th>Amount (°C)</th></tr>
    </thead>
    <tbody>
      <tr>
        <td>1500</td>
        <td><span class="badge ${cls(adj1500)}">${dir(adj1500)}</span></td>
        <td>${mag(adj1500)}</td>
      </tr>
      <tr>
        <td>1520</td>
        <td><span class="badge ${cls(adj1520)}">${dir(adj1520)}</span></td>
        <td>${mag(adj1520)}</td>
      </tr>
    </tbody>
  </table>
